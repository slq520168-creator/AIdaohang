...
function applyChrome(){const s=t();const b=document.getElementById('brand');if(b)b.textContent=s.brand;document.title=s.brand;const ht=document.getElementById('hotTitle');if(ht)ht.textContent=s.hot;if(typeof listTitle!=='undefined'&&listTitle)listTitle.textContent=s.all;if(typeof qEl!=='undefined'&&qEl){qEl.placeholder='';qEl.removeAttribute('placeholder');}if(typeof themeBtn!=='undefined'&&themeBtn)themeBtn.textContent=document.documentElement.dataset.theme==='dark'?s.themeL:s.themeD;if(typeof langBtn!=='undefined'&&langBtn)langBtn.textContent=lang==='en'?'EN':(lang==='zh'?'中文':String(lang).toUpperCase());document.documentElement.lang=lang==='zh'?'zh-CN':lang;}
...
async function load(){selected.clear();tags.clear();applyChrome();const files=['data/tools.json','data/packs.json','data/more.json'];for(let i=2;i<=234;i++)files.push('data/more'+i+'.json');const arrs=await Promise.all(files.map(f=>fetch(f).then(r=>r.ok?r.json():[]).catch(()=>[])));const seenName=new Set();const seenUrl=new Set();tools=[];for(const x of arrs.flat()){if(!x||!x.name||DROP_NAME.has(x.name))continue;if(deadPath(x.url))continue;const h=hostOf(x.url);if(h&&DROP_HOST.has(h))continue;if(seenName.has(x.name))continue;const uk=isHttp(x.url)?urlKey(x.url):'';if(uk&&seenUrl.has(uk))continue;seenName.add(x.name);if(uk)seenUrl.add(uk);tools.push(x);}pickHot();metaEl.textContent=tools.length+t().tools;renderHot();renderSide();render();}
...
const bEl=document.getElementById('brand');if(bEl)bEl.onclick=goTop;
