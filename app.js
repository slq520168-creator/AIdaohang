...
function applyChrome(){const s=t();document.getElementById('brand').textContent=s.brand;document.title=s.brand;document.getElementById('hotTitle').textContent=s.hot;if(listTitle)listTitle.textContent=s.all;qEl.placeholder='';qEl.removeAttribute('placeholder');langBtn.textContent=lang==='zh'?'ZH':'EN';themeBtn.textContent=document.documentElement.dataset.theme==='dark'?s.themeL:s.themeD;document.documentElement.lang=lang==='zh'?'zh-CN':'en';}
...
function render(){const q=(qEl.value||'').trim().toLowerCase();const s=t();const key=[...selected].join(',')+'|'+[...tags].join(',')+'|'+q;if(key!==lastKey){shown=80;lastKey=key;}if(hotBlock)hotBlock.style.display=q?'none':'block';if(listTitle)listTitle.textContent=s.all;if(popEl){popEl.classList.remove('cover');popEl.hidden=!(selected.size||tags.size||q);}metaEl.textContent=tools.length+s.tools;filtered=tools.filter(x=>{const hit=!q||[x.name,x.desc,x.desc_en||'',x.cat,x.how||'',x.url||''].join(' ').toLowerCase().includes(q);return hit&&(q||(matchGroup(x)&&matchTag(x)));});countEl.textContent=filtered.length+s.hit;listEl.innerHTML=filtered.slice(0,shown).map(card).join('')||`<p class="count">${s.empty}</p>`;}
