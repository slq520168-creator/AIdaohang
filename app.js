...
const SNAP='aid_snap_v1';function paintSnap(){try{const o=JSON.parse(localStorage.getItem(SNAP)||'null');if(!o||!Array.isArray(o.cards))return;if(Array.isArray(o.hot)&&o.hot.length){hotList=o.hot;renderHot();}const s=t();metaEl.textContent=o.n+s.tools;countEl.textContent=o.n+s.hit;listEl.innerHTML=o.cards.map(card).join('');}catch(e){}}function saveSnap(){try{if(selected.size||tags.size||(qEl.value||'').trim())return;localStorage.setItem(SNAP,JSON.stringify({n:tools.length,hot:hotList,cards:filtered.slice(0,80)}));}catch(e){}}
