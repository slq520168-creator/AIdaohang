...
function gidOf(x){const s=blob(x);if(/18|Adult|成人|无审核|约炮|出轨|Chaturbate|Pornhub/i.test(s))return 'adult';if(/学习|教育|Khan|Coursera|教程/.test(s))return 'learn';if(/接单|兼职|招聘|问卷/.test(s))return 'gig';if(/绘画|设计/.test(s))return 'draw';if(/视频|音乐|写作/.test(s))return 'make';if(/交友|美食|外卖|陪护|二手|跑腿|维修|美容|附近|医疗|房产|酒店|理财|保险|汽车|天气|地图|体育|时尚|家居|摄影|电子书|播客|农业|租车|电影/.test(s))return 'life';if(/办公|编程|管理|API|物流|会计|建筑|科学|支付|税务|会议|日历|存储|签名|调查|统计|能源|制造|航空|海事|法律|翻译|矩阵|群发|多开|群控|云手机|指纹/.test(s))return 'work';return 'other';}
...
function sortedTags(){return TAGS.slice().sort((a,b)=>{const A=lang==='zh'?a[0]:a[1];const B=lang==='zh'?b[0]:b[1];const la=tagLen(A),lb=tagLen(B);if(la!==lb)return la-lb;return A.localeCompare(B,lang==='zh'?'zh':'en');});}
