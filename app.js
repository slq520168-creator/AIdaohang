...
async function load(){selected.clear();tags.clear();applyChrome();renderSide();const files=['data/tools.json','data/packs.json','data/more.json'];for(let i=2;i<=234;i++)files.push('data/more'+i+'.json');const arrs=await mapPool(files,8,f=>fetchJson(f));const seenName=new Set();const seenUrl=new Set();tools=[];for(const x of arrs.flat()){if(!x||!x.name||DROP_NAME.has(x.name))continue;if(deadPath(x.url))continue;const h=hostOf(x.url);if(h&&DROP_HOST.has(h))continue;if(seenName.has(x.name))continue;const uk=isHttp(x.url)?urlKey(x.url):'';if(uk&&seenUrl.has(uk))continue;seenName.add(x.name);if(uk)seenUrl.add(uk);tools.push(x);}pickHot();metaEl.textContent=tools.length+t().tools;renderHot();renderSide();render();}
