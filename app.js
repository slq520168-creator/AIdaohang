...
function render(){const q=(qEl.value||'').trim().toLowerCase();const s=t();const key=[...selected].join(',')+'|'+[...tags].join(',')+'|'+q;if(key!==lastKey){shown=80;lastKey=key;}if(hotBlock)hotBlock.style.display=q?'none':'block';if(listTitle)listTitle.textContent=s.all;if(popEl){popEl.hidden=!(selected.size||tags.size||q);popEl.classList.remove('cover');}var under=document.getElementById('under');if(under)under.classList.remove('open');metaEl.textContent=tools.length+s.tools;if(!(selected.size||tags.size||q)){filtered=[];countEl.textContent='';listEl.innerHTML='';return;}filtered=tools.filter(x=>{const hit=!q||[x.name,x.desc,x.desc_en||'',x.cat,x.how||'',x.url||''].join(' ').toLowerCase().includes(q);return hit&&(q||(matchGroup(x)&&matchTag(x)));});countEl.textContent=filtered.length+s.hit;listEl.innerHTML=filtered.slice(0,shown).map(card).join('')||`<p class="count">${s.empty}</p>`;try{popEl&&popEl.scrollIntoView({block:'start'});}catch(err){}}
