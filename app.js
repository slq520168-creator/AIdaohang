const I18N={
  zh:{brand:'全球优选AI导航',hot:'今日热门',all:'AI 工具大全',ph:'搜 学习 / 插件 / Grok',themeD:'深色',themeL:'浅色',tools:' 款工具',hit:' 条',empty:'没有匹配',res:'搜索结果 '},
  en:{brand:'Global AI Directory',hot:'Trending',all:'All tools',ph:'Search learn / plugin',themeD:'Dark',themeL:'Light',tools:' tools',hit:'',empty:'No match',res:'Results '}
};
const CATS=[
...
function matchOne(t0,c){
  ...
  if(c==='学习')return t0.cat==='学习'||/学习|教程|课程|Anki|教育|Clipper|Quizlet|Khan/.test(b);
  if(c==='插件')return t0.cat==='插件'||t0.pack==='插件'||/插件|扩展|Plugin|Extension|Addon/.test(b);
  if(c==='视频')return t0.cat==='视频'||/视频|Video/.test(b);
  if(c==='绘画')return t0.cat==='绘画'||t0.cat==='设计'||/绘画|插画/.test(b);
  if(c==='游戏')return t0.cat==='游戏'||/游戏|Game|Unity|Godot/.test(b);
  if(c==='语音')return t0.cat==='语音'||/语音|配音|TTS|Whisper|Speech/.test(b);
  ...
  if(c==='接单')return t0.cat==='接单'||t0.pack==='接单'||/接单|威客|约稿|Upwork|Fiverr|猪八戒|Gig/.test(b);
  if(c==='社区')return t0.cat==='社区'||/社区|Discord|Reddit|Hugging Face/.test(b);
  if(c==='开店')return t0.cat==='开店'||/Shopify|淘宝|小店|Gumroad|Etsy/.test(b);
  if(c==='采集')return t0.cat==='采集'||/OBS|ShareX|Loom|录屏|采集/.test(b);
  if(c==='机器人')return t0.cat==='机器人'||/ROS|Unitree|机器人/.test(b);
