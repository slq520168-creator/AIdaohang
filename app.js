const UI_I18N={zh:{brand:'全球优选AI导航',hot:'今日热点',all:'全部工具',ph:'',themeD:'深色',themeL:'浅色',tools:' 款',hit:' 条',empty:'没有匹配',res:'搜索 ',close:'关闭'},en:{brand:'Global AI Directory',hot:'Today picks',all:'All tools',ph:'',themeD:'Dark',themeL:'Light',tools:' tools',hit:'',empty:'No match',res:'Results ',close:'Close'}};
const GROUPS=[
{k:'电脑软件',en:'PC',id:'pc'},
...
function t(){if(window.I18N&&window.I18N.t){return {brand:window.I18N.t("brand"),hot:window.I18N.t("hot"),all:window.I18N.t("allTools"),themeD:window.I18N.t("themeD"),themeL:window.I18N.t("themeL"),tools:window.I18N.t("toolsCount"),hit:window.I18N.t("hitCount"),empty:window.I18N.t("empty"),ph:"",res:"",close:"Close"};}return UI_I18N[lang]||UI_I18N.zh}
...
const SNAP='aid_snap_v2';function paintSnap(){try{const o=JSON.parse(localStorage.getItem(SNAP)||'null');if(!o||!Array.isArray(o.cards))return;if(Array.isArray(o.hot)&&o.hot.length){hotList=o.hot;renderHot();}const s=t();metaEl.textContent=o.n+s.tools;countEl.textContent=o.n+s.hit;listEl.innerHTML=o.cards.map(card).join('');}catch(e){}}function saveSnap(){try{if(selected.size||tags.size||(qEl.value||'').trim())return;localStorage.setItem(SNAP,JSON.stringify({n:tools.length,hot:hotList,cards:filtered.slice(0,80)}));}catch(e){}}
async function load(){selected.clear();tags.clear();applyChrome();renderSide();paintSnap();const files=['data/tools.json','data/packs.json','data/more.json'];for(let i=2;i<=234;i++)files.push('data/more'+i+'.json');const arrs=await mapPool(files,8,f=>fetchJson(f));const seenName=new Set();const seenUrl=new Set();tools=[];for(const x of arrs.flat()){if(!x||!x.name||DROP_NAME.has(x.name))continue;if(deadPath(x.url))continue;const h=hostOf(x.url);if(h&&DROP_HOST.has(h))continue;if(seenName.has(x.name))continue;const uk=isHttp(x.url)?urlKey(x.url):'';if(uk&&seenUrl.has(uk))continue;seenName.add(x.name);if(uk)seenUrl.add(uk);tools.push(x);}pickHot();metaEl.textContent=tools.length+t().tools;renderHot();renderSide();render();saveSnap();}
function renderSide(){{const u=document.getElementById('under');if(popEl&&u&&tagsEl&&tagsEl.contains(popEl)){popEl.classList.remove('inrow');u.appendChild(popEl);}}const groups=GROUPS.slice().sort((a,b)=>{const A=lang==='zh'?a.k:a.en;const B=lang==='zh'?b.k:b.en;const la=tagLen(A),lb=tagLen(B);if(la!==lb)return la-lb;return A.localeCompare(B);});sideEl.innerHTML=groups.map(g=>`<button data-c="${g.id}" class="${selected.has(g.id)?'on':''}">${lang==='zh'?g.k:g.en}</button>`).join('');sideEl.onclick=e=>{const b=e.target.closest('button');if(!b)return;const id=b.dataset.c;if(selected.has(id))selected.clear();else{selected.clear();selected.add(id);}tags.clear();shown=80;renderSide();render();};if(!tagsEl)return;tagsEl.innerHTML=sortedTags().map(([zh,en])=>{let lab=zh;if(lang!=='zh'){lab=(en&&!hasHan(en))?en:((TAG_EN[zh]&&!hasHan(TAG_EN[zh]))?TAG_EN[zh]:'');if(!lab)return '';}return `<button data-t="${zh}" class="${tags.has(zh)?'on':''}">${lab}</button>`;}).join('');tagsEl.onclick=e=>{const b=e.target.closest('button');if(!b)return;const k=b.dataset.t;if(tags.has(k))tags.delete(k);else tags.add(k);if(k==='免费')tags.delete('收费');if(k==='收费')tags.delete('免费');shown=80;renderSide();render();};sortLangChips();watchChips(sideEl);watchChips(tagsEl);}
